...
class Sensor:
    def __init__(self, interface=None, config_path="config.json", debug=False):
        ...
        try:
            ...
            log_level = getattr(
                logging,
                self.config.get("GENERAL_SETTINGS", {}).get("LOG_LEVEL", "INFO"),
            )
            ...
            if self.start_time:
                try:
                    with self.display_lock:
                        ...
                        packets_per_second = (
                            self.packet_count / elapsed_time if elapsed_time > 0 else 0
                        )
                        ...
                        self.print_banner()
                        print(
                            f"{Fore.GREEN}[*] Monitoring Network Traffic{Style.RESET_ALL}"
                        )
                        print(
                            f"{Fore.YELLOW}[*] Press Ctrl+C to stop capturing{Style.RESET_ALL}\n"
                        )
                        # Print statistics box
                        print(f"{Fore.CYAN}┌─ Monitoring Statistics ────────────┐")
                        print(
                            f"│{Style.RESET_ALL} Packets: {self.packet_count:<8} Rate: {packets_per_second:>6.2f}/s {Fore.CYAN}  │"
                        )
                        print(
                            f"│{Style.RESET_ALL} Alerts: {Fore.RED}{self.alert_count:<6}{Style.RESET_ALL} DNS Queries: {Fore.BLUE}{self.dns_query_count:<6}{Style.RESET_ALL} {Fore.CYAN}│"
                        )
                        print(
                            f"└────────────────────────────────────┘{Style.RESET_ALL}\n"
                        )
...
╚══════════════════════════════════════════════════════════════╝{Style.RESET_ALL}
    ...
    def add_alert(self, alert_msg, alert_type="INFO"):
        ...
        try:
            with self.display_lock:
                ...
                if alert_type == "ALERT" and alert_settings.get(
                    "ALERT_ACTIONS", {}
                ).get("AUTO_BLOCK", {}).get("ENABLED", False):
    ...
    def handle_notification(self, alert_msg, alert_type):
        ...
        email_config = (
            self.config.get("ALERT_SETTINGS", {})
            .get("ALERT_ACTIONS", {})
    ...
    def check_ip_rules(self, src_ip, dst_ip):
        ...
        if src_ip in ip_rules.get("BLACKLISTED_IPS", []) or dst_ip in ip_rules.get(
            "BLACKLISTED_IPS", []
        ):
            self.add_alert(
                f"Detected traffic from/to blacklisted IP - Src: {src_ip}, Dst: {dst_ip}",
                "ALERT",
            )
            return True
        # Check IP ranges
        for ip_range in ip_rules.get("IP_RANGES_TO_MONITOR", []):
            if self.ip_in_range(src_ip, ip_range) or self.ip_in_range(dst_ip, ip_range):
                self.add_alert(
                    f"Detected traffic in monitored IP range - Src: {src_ip}, Dst: {dst_ip}",
                    "INFO",
                )
                return True
        return False
    ...
    def check_port_rules(self, src_ip, dst_ip, src_port, dst_port):
        ...
        if dst_port in port_rules.get("SUSPICIOUS_PORTS", []):
            self.add_alert(
                f"Suspicious port access detected - Src: {src_ip}:{src_port}, Dst: {dst_ip}:{dst_port}",
                "WARNING",
            )
            return True
        # Port scan detection
    ...
    def check_port_scan(self, src_ip, dst_port):
        ...
        if current_time - tracker["start_time"] <= time_window:
            if len(tracker["ports"]) >= threshold:
                self.add_alert(
                    f"Possible port scan detected from {src_ip} - {len(tracker['ports'])} ports in {time_window}s",
                    "ALERT",
                )
                return True
        else:
    ...
    def check_dns_rules(self, packet, dns_packet):
        ...
        if dns_packet.qr == 0:  # DNS query
            ...
            if query in dns_rules.get("DNS_BLACKLIST", []):
                self.add_alert(
                    f"Query to blacklisted domain detected: {query}", "ALERT"
                )
                return True
            ...
            for tld in dns_rules.get("DNS_MONITORING", {}).get("SUSPICIOUS_TLD", []):
                if query.endswith(tld):
                    self.add_alert(
                        f"Query to suspicious TLD detected: {query}", "WARNING"
                    )
                    return True
            # DGA detection
    ...
    def get_network_adapters(self):
        ...
        for iface_name, iface in IFACES.items():
            if self.debug:
                ...
                print(
                    f"  Description: {getattr(iface, 'description', 'No description')}"
                )
                print(f"  Name: {getattr(iface, 'name', 'No name')}")
                print(
                    f"  Network name: {getattr(iface, 'network_name', 'No network name')}"
                )
                print(f"  GUID: {getattr(iface, 'guid', 'No GUID')}\n")
            if isinstance(iface_name, str):
                name = getattr(iface, "description", iface_name)
                description = getattr(
                    iface, "network_name", getattr(iface, "name", "No description")
                )
                adapters.append(
                    {
                        "name": name,
    ...
    def list_interfaces(self):
        ...
        for idx, adapter in enumerate(adapters):
            ...
            print(
                f"{Fore.CYAN}│{Style.RESET_ALL} [{Fore.GREEN}{idx}{Style.RESET_ALL}] {Fore.YELLOW}{name:<28}{Style.RESET_ALL} {desc:<48}"
            )
    ...
    def start_capture(self):
        try:
            if self.interface is None:
                ...
                if self.interface is None:
                    print(
                        f"{Fore.RED}No interface selected. Exiting...{Style.RESET_ALL}"
                    )
                    return
            self.clear_screen()
            self.print_banner()
            ...
            logging.info(
                f"Blacklisted IPs: {', '.join(self.config['IP_RULES']['BLACKLISTED_IPS'])}"
            )
            logging.info(
                f"Suspicious ports: {', '.join(map(str, self.config['PORT_RULES']['SUSPICIOUS_PORTS']))}"
            )
    ...
    def packet_callback(self, packet):
        ...
        try:
                ...
                if DNS in packet:
                    ...
                    if qname:
                        ...
                        if packet[DNS].qr == 0:  # It's a query
                            logging.info(
                                f"DNS Query: {qname.rstrip('.')} from {src_ip} to {dst_ip} (Type: Query)"
                            )
                        else:  # It's a response
                            # Extract the IP address from the answer section of the DNS response
                            resolved_ip = None
                            if (
                                ...
                                for i in range(packet[DNS].ancount):
                                    ...
                                    if (
                                        answer.type == 1
                                    ):  # Type 1 means A record (IPv4 address)
                                        resolved_ip = answer.rdata
                                        break  # Stop once we find the first A record

                            # Log the DNS query and resolved IP if it's a response
                            if resolved_ip:
                                logging.info(
                                    f"DNS Query: {qname.rstrip('.')} from {src_ip} to {dst_ip} (Resolved IP: {resolved_ip})"
                                )
                            else:
                                logging.info(
                                    f"DNS Query: {qname.rstrip('.')} from {src_ip} to {dst_ip} (Type: Response, No IP resolved)"
                                )
            ...
            self.detailed_logger.log_event(
                event_type="ERROR",
                data={"error_type": "PACKET_PROCESSING_ERROR", "description": str(e)},
                source="system",
            )
