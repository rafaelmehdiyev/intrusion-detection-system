...
USERS = {"admin": generate_password_hash("123")}
...
def is_private_ip(ip):
    """Check if the given IP is private."""
    try:
...
def utility_processor():
    return {"map": map, "str": str, "list": list}
...
@requires_auth
def get_geoip():
    """Fetch geographic location of an IP address."""
    ip_address = request.args.get("ip")
    if not ip_address:
        return jsonify({"error": "IP address is required"}), 400

    if is_private_ip(ip_address):
        # Return a message for private IPs
        return jsonify(
            {
                "ip": ip_address,
    ...
    try:
        # Use ipwhois.app API for GeoIP lookup
        response = requests.get(f"https://ipwhois.app/json/{ip_address}")
        data = response.json()

        if not data.get("success", True):
            return jsonify(
                {"error": data.get("message", "Unable to fetch GeoIP data")}
            ), 404

        # Return country, region, city, and additional GeoIP information
        return jsonify(
            {
                "ip": ip_address,
    ...
    try:
        ...
        if "LOG_DIR" in form_data or "LOG_LEVEL" in form_data:
            ...
            if "CAPTURE_TIMEOUT" in form_data:
                current_config["GENERAL_SETTINGS"]["CAPTURE_TIMEOUT"] = int(
                    form_data["CAPTURE_TIMEOUT"]
                )

        elif "BLACKLISTED_IPS" in form_data or "WHITELISTED_IPS" in form_data:
            # IP Rules
            if "BLACKLISTED_IPS" in form_data:
                current_config["IP_RULES"]["BLACKLISTED_IPS"] = [
                    ip.strip()
                    for ip in form_data["BLACKLISTED_IPS"].split("\n")
                    if ip.strip()
                ]
            if "WHITELISTED_IPS" in form_data:
                current_config["IP_RULES"]["WHITELISTED_IPS"] = [
                    ip.strip()
                    for ip in form_data["WHITELISTED_IPS"].split("\n")
                    if ip.strip()
                ]
            if "IP_RANGES" in form_data:
                current_config["IP_RULES"]["IP_RANGES_TO_MONITOR"] = [
                    ip.strip()
                    for ip in form_data["IP_RANGES"].split("\n")
                    if ip.strip()
                ]
            if "MAX_CONNECTIONS_PER_IP" in form_data:
                current_config["IP_RULES"]["MAX_CONNECTIONS_PER_IP"] = int(
                    form_data["MAX_CONNECTIONS_PER_IP"]
                )
            if "CONNECTION_TIMEOUT" in form_data:
                current_config["IP_RULES"]["CONNECTION_TIMEOUT"] = int(
                    form_data["CONNECTION_TIMEOUT"]
                )

        elif "SUSPICIOUS_PORTS" in form_data or "ALLOWED_PORTS" in form_data:
            # Port Rules
            if "SUSPICIOUS_PORTS" in form_data:
                current_config["PORT_RULES"]["SUSPICIOUS_PORTS"] = [
                    int(port.strip())
                    for port in form_data["SUSPICIOUS_PORTS"].split("\n")
                    if port.strip().isdigit()
                ]
            if "ALLOWED_PORTS" in form_data:
                current_config["PORT_RULES"]["ALLOWED_PORTS"] = [
                    int(port.strip())
                    for port in form_data["ALLOWED_PORTS"].split("\n")
                    if port.strip().isdigit()
                ]
            if "PORT_SCAN_ENABLED" in form_data:
                current_config["PORT_RULES"]["PORT_SCAN_DETECTION"]["ENABLED"] = (
                    form_data["PORT_SCAN_ENABLED"] == "true"
                )
            if "PORT_SCAN_THRESHOLD" in form_data:
                current_config["PORT_RULES"]["PORT_SCAN_DETECTION"]["THRESHOLD"] = int(
                    form_data["PORT_SCAN_THRESHOLD"]
                )
            if "TIME_WINDOW" in form_data:
                current_config["PORT_RULES"]["PORT_SCAN_DETECTION"]["TIME_WINDOW"] = (
                    int(form_data["TIME_WINDOW"])
                )

        elif "DNS_BLACKLIST" in form_data or "SUSPICIOUS_TLD" in form_data:
            # DNS Rules
            if "DNS_BLACKLIST" in form_data:
                current_config["DNS_RULES"]["DNS_BLACKLIST"] = [
                    domain.strip()
                    for domain in form_data["DNS_BLACKLIST"].split("\n")
                    if domain.strip()
                ]
            if "DGA_DETECTION_ENABLED" in form_data:
                current_config["DNS_RULES"]["DGA_DETECTION"]["ENABLED"] = (
                    form_data["DGA_DETECTION_ENABLED"] == "true"
                )
            if "MIN_ENTROPY" in form_data:
                current_config["DNS_RULES"]["DGA_DETECTION"]["MIN_ENTROPY"] = float(
                    form_data["MIN_ENTROPY"]
                )
            if "MIN_LENGTH" in form_data:
                current_config["DNS_RULES"]["DGA_DETECTION"]["MIN_LENGTH"] = int(
                    form_data["MIN_LENGTH"]
                )
            if "CONSONANT_THRESHOLD" in form_data:
                current_config["DNS_RULES"]["DGA_DETECTION"]["CONSONANT_THRESHOLD"] = (
                    float(form_data["CONSONANT_THRESHOLD"])
                )
            if "CACHE_TIMEOUT" in form_data:
                current_config["DNS_RULES"]["DNS_MONITORING"]["CACHE_TIMEOUT"] = int(
                    form_data["CACHE_TIMEOUT"]
                )
            if "MAX_QUERIES_PER_DOMAIN" in form_data:
                current_config["DNS_RULES"]["DNS_MONITORING"][
                    "MAX_QUERIES_PER_DOMAIN"
                ] = int(form_data["MAX_QUERIES_PER_DOMAIN"])
            if "SUSPICIOUS_TLD" in form_data:
                current_config["DNS_RULES"]["DNS_MONITORING"]["SUSPICIOUS_TLD"] = [
                    tld.strip()
                    for tld in form_data["SUSPICIOUS_TLD"].split("\n")
                    if tld.strip()
                ]
    ...
    try:
        ...
        if os.path.exists(log_file):
            with open(log_file, "r") as f:
                logs = f.readlines()
                return jsonify(
                    {"logs": [log.strip() for log in logs], "count": len(logs)}
                )
...
def get_logs_for_date(date):
    ...
    try:
        ...
        with open(log_file, "r") as f:
            logs = f.readlines()
            return jsonify({"logs": [log.strip() for log in logs], "count": len(logs)})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
